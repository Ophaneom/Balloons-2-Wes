...
local function OnDeath(inst)
    ...
    SpawnPrefab("pigking_wes").Transform:SetPosition(inst.Transform:GetWorldPosition())
    SpawnPrefab("collapse_big").Transform:SetPosition(inst.Transform:GetWorldPosition())

    local x, y, z = inst.Transform:GetWorldPosition()
    for i = 1, TUNING.BTWS_PIGMAN_QNT do
        local randx = x + math.random(- TUNING.BTWS_PIGMAN_RANGE, TUNING.BTWS_PIGMAN_RANGE) 
        local randz = z + math.random(- TUNING.BTWS_PIGMAN_RANGE, TUNING.BTWS_PIGMAN_RANGE) 
        SpawnPrefab("collapse_small").Transform:SetPosition(randx,y,randz)
        SpawnPrefab("pigman_wes").Transform:SetPosition(randx,y,randz)
    end
...
---------------------------------------------------------------------------------------------------------------------------------------
    ...
    --//
    repeatcount = 0
    inst.task = inst:DoPeriodicTask(TUNING.BTWS_DELAY_PIG_BALLOON, WaitExplosion)
...
return Prefab("balloon_pig", fn, assets)
