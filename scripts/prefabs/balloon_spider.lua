...
local function OnDeath(inst)
    ...
    local x, y, z = inst.Transform:GetWorldPosition()
    for i = 1, TUNING.BTWS_SPIDERDEN_QNT do
        local randx = x + math.random(- TUNING.BTWS_SPIDERDEN_RANGE, TUNING.BTWS_SPIDERDEN_RANGE) 
        local randz = z + math.random(- TUNING.BTWS_SPIDERDEN_RANGE, TUNING.BTWS_SPIDERDEN_RANGE) 
        SpawnPrefab("collapse_small").Transform:SetPosition(randx,y,randz)
        SpawnPrefab("spiderden").Transform:SetPosition(randx,y,randz)
    end
...
---------------------------------------------------------------------------------------------------------------------------------------
    ...
    --//
    repeatcount = 0
    inst.task = inst:DoPeriodicTask(TUNING.BTWS_DELAY_SPIDER_BALLOON, WaitExplosion)
...
return Prefab("balloon_spider", fn, assets)
