...
local AREAATTACK_EXCLUDETAGS = { "INLIMBO", "notarget", "noattack", "flight", "invisible", "playerghost", "mime" }
...
local function OnDeath(inst)
    ...
    local x, y, z = inst.Transform:GetWorldPosition()
    for i = 1, TUNING.BTWS_BEEFALO_QNT do
        if TUNING.BTWS_BEEFALO_QNT == 1 then
            SpawnPrefab("babybeefalo").Transform:SetPosition(x, y, z)
        else
            local randx = x + math.random(- TUNING.BTWS_BEEFALO_RANGE, TUNING.BTWS_BEEFALO_RANGE) 
            local randz = z + math.random(- TUNING.BTWS_BEEFALO_RANGE, TUNING.BTWS_BEEFALO_RANGE) 
            SpawnPrefab("collapse_small").Transform:SetPosition(randx,y,randz)
            SpawnPrefab("babybeefalo").Transform:SetPosition(randx,y,randz)
        end
    end
...
------------------------------------------------------------------------------------------------------------------
local function WaitExplosion(inst)
    inst.task = inst:DoPeriodicTask(1, OnDeath)
end
------------------------------------------------------------------------------------------------------------------
local function fn()
...
---------------------------------------------------------------------------------------------------------------------------------------
    ...
    --//
    repeatcount = 0
    inst.task = inst:DoPeriodicTask(TUNING.BTWS_DELAY_BEEFALO_BALLOON, WaitExplosion)
...
return Prefab("balloon_beefalo", fn, assets)
